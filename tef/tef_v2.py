...
def haversine(lon1, lat1, lon2, lat2):
    ...
    m = 6371000* c #meters
    return m
#function based on tef.py but with the numeric formulas of Burchard and MacCready (2018 in prep.)
def TEF_salt_v2(ncfile,t,lat_or_lon,lat_or_lon_index,i_start,i_end,exponent=7,s='auto',outputtype='normal',exchange_out=True,time_mean='no',land_depth=-10,variable_list=[],heat_exchange=False,rotation=True,slice_k=[]): #args: time_mean
	...
	if s == 'auto':
		N = np.power(2,exponent)
		DeltaS = np.float64((s_max-s_min)/N)
	...
	if not type(t) is int: #if t is not an index, we have a higher dimension
		...
		dqds = np.zeros(shape=(np.shape(salt)[0],len(s)+2),dtype=np.float64)
		dq_sds = np.zeros(shape=(np.shape(salt)[0],len(s)+2),dtype=np.float64)
		if heat_exchange:
			heat_cont = np.zeros(shape=np.shape(dqds),dtype=np.float64) #unit: J m-3
		#print(np.shape(salt))
		...
		while tt < np.shape(salt)[0]: #time
			...
			while i < np.shape(salt)[2]: #lon/lat
				...
				while k < np.shape(salt)[1]: # depth
					#calc into which bin 
					n = int((salt[tt,k,i]-s_min)/(s_max-s_min)*N)+1
					#print(n)
					if heat_exchange:
						heat_cont[tt,n] = heat_cont[tt,n]+h[tt,k,i]*temp[tt,k,i]*dens[tt,k,i]*c_p*np.sign(vel[tt,k,i]) #np.sign to have minus sign if vel is negative
					dqds[tt,n] = dqds[tt,n] + vel[tt,k,i]*h[tt,k,i]*ds[i]/DeltaS
					dq_sds[tt,n] = dq_sds[tt,n] + vel[tt,k,i]*h[tt,k,i]*ds[i]*salt[tt,k,i]/DeltaS
					k+=1
				i+=1
			tt+=1
		...
		#print(len(s_new))
		#print(dqds)
		dqds = dqds[:,1:-1] #get rid of +2 from initialization
		dq_sds = dq_sds[:,1:-1]		
		...
		if exchange_out:
			...
			while tt < np.shape(dqds)[0]:
				...
				Q_in[tt] = np.sum((dqds[tt,:]*DeltaS).clip(min=0))
				Q_out[tt] = np.sum((dqds[tt,:]*DeltaS).clip(max=0))
				S_in[tt] = np.sum((dq_sds[tt,:]).clip(min=0))*DeltaS # Salinity flux
				S_out[tt] = np.sum((dq_sds[tt,:]).clip(max=0))*DeltaS # Salinity flux
			...
			if not heat_exchange:
				if time_mean == 'no':
					return(dqds, dq_sds, s, ex_out)
				else:
					return(dqds, dq_sds, s, ex_out, time_indices)
			if heat_exchange:
				#calculate stuff:
				...
				tt=0
				while tt < np.shape(dqds)[0]:
					Heat_in[tt] = np.sum(heat_cont[tt,:].clip(min=0)) # J/s
					Heat_out[tt] = np.sum(heat_cont[tt,:].clip(max=0)) # J/s
					tt+=1
		...
		dqds = np.zeros(shape=(len(s)+2,),dtype=np.float64) 
		dq_sds = np.zeros(shape=(len(s)+2,),dtype=np.float64)
		if heat_exchange:
			heat_cont = np.zeros(shape=np.shape(dqds),dtype=np.float64) #unit: J m-3
		#print(np.shape(salt))
		#print(np.shape(Q_iso))
		...
		while i < np.shape(salt)[1]: #lon/lat
			...
			while k < np.shape(salt)[0]: # depth
				n = int((salt[k,i]-s_min)/(s_max-s_min)*N)+1
				if heat_exchange:
					heat_cont[n] = heat_cont[n]+h[k,i]*temp[k,i]*dens[k,i]*c_p*np.sign(vel[k,i]) #np.sign to have minus sign if vel is negative
				dqds[n] = dqds[n] + vel[k,i]*h[k,i]*ds[i]/DeltaS
				dq_sds[n] = dq_sds[n] + vel[k,i]*h[k,i]*ds[i]*salt[k,i]/DeltaS
				k+=1
			i+=1
		...
		if not exchange_out:
			if time_mean == 'no':
				return(dqds, dq_sds, s)
			else:
				return(dqds, dq_sds, s,time_indices)
		...
		if exchange_out:
			...
			Q_in = np.sum((dqds*DeltaS).clip(min=0))
			Q_out = np.sum((dqds*DeltaS).clip(max=0))
			S_in = np.sum((dq_sds).clip(min=0))*DeltaS # Salinity flux
			S_out = np.sum((dq_sds).clip(max=0))*DeltaS # Salinity flux
			s_in = S_in/Q_in
			s_out = S_out/Q_out
			...
			if not heat_exchange:
				if time_mean == 'no':
					...
					return(dqds, dq_sds, s, ex_out,time_indices)
			if heat_exchange:
				Heat_in = np.sum(heat_cont.clip(min=0))
				Heat_out = np.sum(heat_cont.clip(max=0))
				ex_out.append(Heat_in)
				ex_out.append(Heat_out)
				if time_mean == 'no':
					return(dqds, dq_sds, s, ex_out)
